...
class GraphEModel(nn.Module):
    ...
    def __init__(self, model_config):
        ...
        # build symmetric normalized Laplacian L = I - D^{-1/2} A D^{-1/2}, where A includes self-loops
        A_loops = self.full_A + torch.eye(self.full_A.size(0))
        deg = A_loops.sum(dim=1)
        deg_inv_sqrt = deg.pow(-0.5)
        deg_inv_sqrt[torch.isinf(deg_inv_sqrt)] = 0.0        # handle any inf values
        normA = deg_inv_sqrt.view(-1, 1) * A_loops * deg_inv_sqrt.view(1, -1)
        
        self.full_X = A_loops  

        # adjacency + self-loops for GCN input (already included in A_loops)
        ...
        self.autoEncoder['net'] = GraphAutoencoder(
            ...
            K          = model_config['appnp_K'],
            alpha      = model_config['appnp_alpha'],
            dropout    = model_config['appnp_dropout']
        )
        # ── Optimizers ──
        self.optimizatior = {}
        ...
        for phase in phases_list:
            ...
            if phase == 'net':
                for epoch in range(1, epochs_time+1):
                    ...
                    loss_values_matrix = {
                        "net": {
                            "edge_index":  self.full_edge_index,           # sparse structure
                            "edge_weight": self.full_edge_weight,          # |E| ground‑truth
                            "y_true":      self.full_edge_weight,          # alias for your 2‑order term
                            "y_pred":      recon,                          # |E| predictions
                            "y_late":      Z,                              # N×d embedding
                            "B_param":     torch.ones_like(self.full_edge_weight),
                            "y_adj":       self.full_A                    # N×N dense adjacency
                        },
                            "att": {
                                "y_true":  None,
                                "y_pred":  None,
