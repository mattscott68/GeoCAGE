        ...
        for phase in phases_list:
            ...
            if phase == 'net':
                for epoch in range(1, epochs_time+1):
                    ...
                    print(f"Phase: net\tEpoch: {epoch_global}/{tot_epochs}\tLoss: {loss_val}")
                    self.checkpointer.checkToSave(self, epoch_global, tot_epochs, loss_val, phase='net')

                # store embeddings & continue
                self.set_embedding(encoder_out={ 'net': outputs.get('net',[]) },
                                last_epoch='net', save=True,
                                path=path_embedding, phases=['net'])
                outputs['net'] = []
                continue
            ...
            for epoch in range(epochs_time):
                ...
                for [input, B_param, batch_info] in datagenerator.generate():
                    ...
                    comp = self.autoEncoder['att'].forward(att_batch)
                    loss_values_matrix = {
                        "net": {
                            "y_true": None,
            ...
            self.set_embedding(encoder_out=outputs, last_epoch='att', save=True, path=path_embedding, phases=['att'])
